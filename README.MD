...
- [Что такое конструктор?](https://github.com/TwITe/OOP-Tutorial#%D0%A7%D1%82%D0%BE-%D1%82%D0%B0%D0%BA%D0%BE%D0%B5-%D0%BA%D0%BE%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BA%D1%82%D0%BE%D1%80)
    ...
    - [Initialization list](https://github.com/TwITe/OOP-Tutorial#initialization-list)
...
### Initializer list
**Initializer list** требуется для того, чтобы инициализировать переменные класса. Его семантика ничем не отличается от инциализации внутри тела
конструктора. Однако, в некоторых ситуациях требуется именно данный метод инициализации членов класса. Данные случаи будут рассмотрены ниже.
...
private:
    int a, b;
public:
    Base(int i, int j) : a(i), b(j) {}; // Initializer list
...
private:
    const int s;
public:
    Base (int i = 0): s(i) {}; // Верный вариант
/*    Base(int i = 0) { // Ошибка
        s = i;
    };*/
...
private:
    A a_object;
public:
    B(int a): a_object(a) { // Инициализация объекта a_object с помощью parameterized конструктора класса A
        // Класс A не имеет стандартного конструктора и не был автоматически создан.
...
public:
    Base(int i): i(i) {
        cout << "Data member i was initialized to: " <<  i;
    };
};

int main() {
    Base base(10); // Data member i was initialized to: 10
    return 0;
}
```
...
## Геттеры & Сеттеры
**Getter** - метод класса, возвращающий данные обратно требуемые данные из класса
...
int main() {
    Base base;
    base.set_value(5);
    cout << "The variable was setted to: " << base.get_value(); // The variable was setted to: 5
    return 0;
}
```
...
Класс также можно разделить на описание и саму реализацию и подключать их в виде библиотеки.
Пример:

Содержимое файла **Base.h:**. Описание класса.
```c++
#ifndef OOP_LEARN_BASE_H
#define OOP_LEARN_BASE_H
...
Содержимое файла **Base.cpp**. Имплементация класса.
```c++
#include <iostream>
#include "Base.h"
using namespace std;
...
Base:: Base() {
    s = 0;
    cout << "Constructor was called in Base header file" << endl;
}

int Base::set_value(int a) {
    s = a;
}
...
int main() {
    Base base; // Constructor was called in Base header file
    base.set_value(5);
    cout << "The variable was setted to: " << base.get_value(); // The variable was setted to: 5
    return 0;
}
...
- Легче проделывать какие-либо изменения в коде, так как для этого потребуется только найти нужные файлы, где реализован класс, а не искать среди
 огромного количества строк кода(в больших проектах).